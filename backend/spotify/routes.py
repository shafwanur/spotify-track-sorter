...
@router.get("/login")  
def login():
    # TODO: should probably be a post request, but FastAPI doesn't care, and I'm ignoring it for now.
...
def auth_callback(request: Request):
    ...
    return JSONResponse(
        {  # TODO: would making a call to /spotify/login return this json?
...
@router.post(
    "/token/refresh"
)  # expects the auth_code returned from /success to generate refresh_token. or if it exists (in memory in the dict), just returns it. refresh_tokens are permanent.
def retrieve_refresh_token(
    payload: RefreshTokenRequest,
):  # TODO: should probably be async functions ... will change later.
    if (
        payload.auth_code
    ):  # if auth_code passed, generate new refresh_token, store in db
